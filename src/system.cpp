...
void initExpander(Expander *expander, uint8_t I2CAddress, uint8_t IOdirA,
  ...
  if (wiringPiI2CWriteReg8(expander->fd, expander->IOdirA,
                           expander->inputsDirA) == -1) {
    printf("[ERROR] - Cannot set IO for Port A direction\n");
    exit(EXIT_FAILURE);
  }
  if (wiringPiI2CWriteReg8(expander->fd, expander->IOdirB,
                           expander->inputsDirB) == -1) {
  ...
  expander->inputsDirA = inputsDirA;
  expander->inputsDirB = inputsDirB;
  if (wiringPiI2CWriteReg8(expander->fd, expander->portGPPUA,
                           expander->inputsDirA) == -1) {
    printf("[ERROR] - Cannot set IO for Port A direction\n");
    exit(EXIT_FAILURE);
  }
  if (wiringPiI2CWriteReg8(expander->fd, expander->portGPPUB,
                           expander->inputsDirB) == -1) {
    printf("[ERROR] - Cannot set IO for Port B direction\n");
    exit(EXIT_FAILURE);
  }
...
void initCubeSystem(CubeSystem *system) {
  ...
  initExpander(&system->Expander1, MCP23017_I2C_ADDRESS_1, IODIRA, IODIRB,
               GPIOA, GPIOB, GPPUA, GPPUB,
               system->Button11.pin | system->Button12.pin,
               system->Button21.pin | system->Button22.pin, 0x00, 0x00);

  initExpander(&system->Expander2, MCP23017_I2C_ADDRESS_2, IODIRA, IODIRB,
               GPIOA, GPIOB, GPPUA, GPPUB,
               system->Button13.pin | system->Button14.pin,
               system->Button23.pin | system->Button24.pin, 0x00, 0x00);

  initExpander(&system->Expander3, MCP23017_I2C_ADDRESS_3, IODIRA, IODIRB,
               GPIOA, GPIOB, GPPUA, GPPUB,
               system->Button15.pin | system->Button16.pin,
               system->Button25.pin | system->Button26.pin, 0x00, 0x00);
